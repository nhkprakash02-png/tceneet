...
import { NEET_SUBJECTS } from '../lib/utils';
...
export default function Quiz() {
  ...
  const startQuiz = (qCount) => {
    ...
    // Mixed-subject pool across all 4 NEET subjects (there is no "GK" subject in NEET) —
    // whatever the admin has uploaded via the Quiz Question Uploader, across Physics,
    // Chemistry, Botany and Zoology, is fair game for a Quick Quiz question.
    const mixedPool = DB.quizPool.filter((q) => NEET_SUBJECTS.includes(q.subject)).map(normalizeQuestion).filter(isUsableQuestion);
    if (!mixedPool.length) { alert('No usable quiz questions are available yet. Please check back soon.'); return; }
    const pool = shuffle(mixedPool).slice(0, Math.min(qCount, mixedPool.length));
    const mins = quizDurationMinutes(pool.length);
    const test = {
      id: 'quiz_' + pool.length, title: `Quick Quiz — Mixed Subjects — ${pool.length} Questions`,
      durationMin: mins, marksCorrect: 4, marksWrong: 1, questions: pool,
    };
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="muted text-sm">Physics, Chemistry, Botany &amp; Zoology — questions are pulled randomly across all 4 NEET subjects.</p>
      </div>
      ...
      <div className="card glow-border rounded-2xl p-10 sm:p-16 text-center max-w-xl mx-auto">
        ...
        <h3 className="font-display font-700 text-lg mb-2">Ready for a quick NEET quiz?</h3>
