...
const SUBJECTS = [['physics', 'Physics'], ['chemistry', 'Chemistry'], ['botany', 'Botany'], ['zoology', 'Zoology'], ['full', 'Full Mock']];
...
export default function MockTest() {
  ...
  const shareTestToWhatsApp = (t) => {
    const url = window.location.origin + testDeepLinkPath(t.id);
    const text = `Check out "${t.title}" on TCE - NEET: ${url}`;
    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="muted text-sm">Official NEET pattern — Section A (35 mandatory) + Section B (any 10 of 15) with +4/-1 marking. Free demo mock in every subject, rest unlocked for enrolled batch students only.</p>
        </div>
