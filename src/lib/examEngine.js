...
// --- NEET exam pattern -------------------------------------------------------------------
// Official NEET pattern per subject: Section A has 35 MANDATORY questions; Section B has 15
// questions of which only ANY 10 need to be (and are scored if) attempted. Questions carry a
// `section: 'A' | 'B'` tag (set in the Question Editor / seed data); anything untagged is
// treated as Section A for backward compatibility with older PYQ/Quiz data that predates this
// pattern (those still work exactly as before — every question mandatory, all of them scored).
export const NEET_SECTION_A_SIZE = 35;
export const NEET_SECTION_B_SIZE = 15;
export const NEET_SECTION_B_ANSWER_LIMIT = 10;

export function questionSection(q) { return q && q.section === 'B' ? 'B' : 'A'; }

// Keeps Section A and Section B as two separate blocks (matching the real NEET paper layout —
// Section B always follows Section A) while still shuffling the questions WITHIN each section,
// so repeated attempts don't show questions in a memorizable fixed order.
export function shuffleBySections(questions) {
  const a = questions.filter((q) => questionSection(q) === 'A');
  const b = questions.filter((q) => questionSection(q) === 'B');
  if (!b.length) return shuffle(questions); // no Section B tagging present — behave exactly as before
...
export function createExamState(test, source, subject, chosenLang) {
  const shuffledQ = shuffleBySections(test.questions);
...
export function sectionDisplayName(subject) {
  const map = { physics: 'Physics', chemistry: 'Chemistry', botany: 'Botany', zoology: 'Zoology', full: 'Full Syllabus (All Subjects)', quiz: 'Mixed Subject Quiz', pyq: 'Previous Year Section' };
...
// Determines, for each question index, whether it counts toward scoring — implements the
// NEET "Section B: answer any 10 of 15" rule. Section A questions always count. Within
// Section B, only the FIRST `NEET_SECTION_B_ANSWER_LIMIT` questions (in on-screen order) that
// were actually answered count; anything answered beyond that limit is excluded entirely
// (neither correct nor wrong, not counted as attempted) — exactly like a real NEET OMR/CBT
// which only evaluates the first 10 Section B responses recorded. If a test has no Section B
// questions at all, every question counts (unchanged behavior for older PYQ/Quiz data).
export function scoredQuestionMask(questions, answers) {
  let sectionBScoredSoFar = 0;
  return questions.map((q, i) => {
    if (questionSection(q) !== 'B') return true;
    const answered = answers[i] !== null && answers[i] !== undefined;
    if (!answered) return true; // an unanswered Section B slot doesn't affect anyone's count
    sectionBScoredSoFar += 1;
    return sectionBScoredSoFar <= NEET_SECTION_B_ANSWER_LIMIT;
  });
}

// Scores a finished exam and builds the submission record (mirrors finishExam()'s scoring half).
// Marking scheme is read from the test itself (marksCorrect / marksWrong) — new Mock/PYQ tests
// default to the official NEET scheme of +4 for correct and -1 for an incorrect answer (see
// MockManager.jsx / PyqManager.jsx), but this stays generic so older non-NEET data keeps working.
export function buildSubmission(examState, DB, user) {
  const st = examState;
  const scoredMask = scoredQuestionMask(st.questions, st.answers);
  let correct = 0, wrong = 0, unanswered = 0, scoredCount = 0;
  st.questions.forEach((q, i) => {
    if (!scoredMask[i]) return; // excess Section-B attempt beyond the "any 10 of 15" cap — not scored
    scoredCount++;
  ...
  const maxScore = +(scoredCount * st.marksCorrect).toFixed(2);
...
// attempts: paid mock tests only (no quiz, no PYQ, no free-demo mocks), any exempt accounts
// (see EXEMPT_ADMIN_EMAILS in lib/utils.js — empty by default in this NEET template) fully
// excluded, and only each student's genuine first attempt per test.
...
export function drawScorecardCanvas(sub, rank) {
  ...
  ctx.fillStyle = goldGrad; ctx.font = 'bold 34px Arial'; ctx.fillText('TCE - NEET', 40, 70);
  ...
  ctx.fillText('TCE - NEET | Update your institute address in src/lib/examEngine.js', 40, 520);
  return canvas;
}
export function downloadScorecard(sub, rank) {
  const canvas = drawScorecardCanvas(sub, rank);
  const a = document.createElement('a'); a.href = canvas.toDataURL('image/png'); a.download = 'TCE_NEET_Scorecard_' + sub.studentName.replace(/\s+/g, '_') + '.png'; a.click();
}
...
export function printOfflinePaper(test) {
  ...
  <div style="font-family:Arial, sans-serif; color:#000; padding:20px; max-width:800px; margin:0 auto;">
    <h2 style="text-align:center;margin-bottom:4px;">TCE - NEET</h2>
    <p style="text-align:center;margin-top:0;font-size:12px;">Update your institute address/contact in src/lib/examEngine.js (printOfflinePaper)</p>
    <hr>
    <h3>${test.title}</h3>
    ...
    ${test.questions.map((q, i) => `
      <div style="margin-bottom:14px; page-break-inside:avoid;">
        <p style="font-weight:bold; margin-bottom:4px;">${i + 1}. [Sec ${questionSection(q)}] ${q.textEn} ${q.textBn ? ('&nbsp; / ' + q.textBn) : ''}</p>
        ${q.image ? `<img src="${q.image}" style="max-width:320px;max-height:220px;display:block;margin:6px 0;" />` : ''}
        <div style="display:flex; flex-wrap:wrap; gap:16px; font-size:13px;">
          ${q.options.map((o) => `<span>(${o.key}) ${o.textEn}</span>`).join('')}
        </div>
      </div>`).join('')}
    <hr>
    <p style="font-size:11px; text-align:center;">© TCE - NEET</p>
  </div>`;
