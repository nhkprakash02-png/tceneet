...
export async function buildWatermarkedPdfBytes(mat, user) {
  ...
  const licenseText = `Licensed to: ${user ? user.name : 'Guest'} | ${user ? (user.phone || user.email) : ''} — TCE - NEET`;
  ...
  page.drawText('TCE - NEET', { x: 50, y: 780, size: 20, font, color: rgb(0.83, 0.55, 0.05) });
