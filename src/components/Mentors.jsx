...
export default function Mentors() {
  const { DB } = useApp();
  if (!DB.mentors || !DB.mentors.length) return null; // clean placeholder — nothing renders until faculty are added via Admin Panel
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-5 gap-y-10">
        {DB.mentors.map((m) => {
          // Photos are uploaded per-mentor via Admin Panel → Mentors / Faculty and stored as
          // Base64 directly on the mentor record (m.photo) — no static image files or
          // hardcoded filenames, so a new institute's faculty photos "just work" the moment
          // they're uploaded, and a gold initials avatar is shown until then.
