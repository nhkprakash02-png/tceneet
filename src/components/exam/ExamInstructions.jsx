...
export default function ExamInstructions({ test, user, resumeData, onBegin, onResume, onCancel }) {
  ...
  const sectionACount = test.questions.filter((q) => questionSection(q) === 'A').length;
  const sectionBCount = test.questions.filter((q) => questionSection(q) === 'B').length;
  const hasSectionB = sectionBCount > 0;
  const scoredQuestionCount = sectionACount + Math.min(sectionBCount, NEET_SECTION_B_ANSWER_LIMIT);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 modal-backdrop">
      <div className="card glow-border rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="card2 rounded-xl p-4 mb-4 text-sm space-y-2">
          ...
          <p><b>Total Questions:</b> {test.questions.length}{hasSectionB && ` (Section A: ${sectionACount} mandatory, Section B: ${sectionBCount} — answer any ${NEET_SECTION_B_ANSWER_LIMIT})`}</p>
          <p><b>Duration:</b> {test.durationMin} minutes</p>
          <p><b>Total Marks:</b> {+(scoredQuestionCount * test.marksCorrect).toFixed(2)}</p>
        ...
        <ul className="text-xs muted space-y-1.5 mb-4 list-disc pl-4">
          ...
          {hasSectionB && <li>Section A ({sectionACount} questions) is compulsory. In Section B ({sectionBCount} questions), only your first {NEET_SECTION_B_ANSWER_LIMIT} answered questions will be evaluated — exactly as in the real NEET exam.</li>}
        </ul>
        <div className="mb-4">
