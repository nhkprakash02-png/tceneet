...
import { questionSection, NEET_SECTION_B_ANSWER_LIMIT } from '../../lib/examEngine';
...
export default function ExamRunner({ initialExam, user, onFinish }) {
  ...
  // NEET Section A/B awareness — Section A is mandatory, Section B has 15 questions of which
  // only the first 10 answered (in on-screen order) actually get scored (see examEngine.js
  // scoredQuestionMask). Purely informational here; the answer-count cap itself is enforced at
  // scoring time so a student can still freely browse/change Section B answers during the exam.
  const sectionAIdx = st.questions.map((_, i) => i).filter((i) => questionSection(st.questions[i]) === 'A');
  const sectionBIdx = st.questions.map((_, i) => i).filter((i) => questionSection(st.questions[i]) === 'B');
  const hasSectionB = sectionBIdx.length > 0;
  const currentSection = questionSection(q);
  const sectionBAnswered = sectionBIdx.filter((i) => st.answers[i] !== null && st.answers[i] !== undefined).length;
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'var(--bg)' }}>
      <div className="flex items-center justify-between px-4 py-3 card2" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-xs font-semibold">TCE - NEET — {st.title}</p><p className="text-[10px] muted">Candidate: {user.name}</p></div>
        </div>
        <div className="flex items-center gap-3">
          {hasBn && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
            ...
            {hasSectionB && (
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                <span className={`badge ${currentSection === 'B' ? 'bg-purple-500/15 text-purple-400' : 'bg-emerald-500/15 text-emerald-400'}`}>Section {currentSection}</span>
                {currentSection === 'B' && <span className="text-[11px] muted">Answer any {NEET_SECTION_B_ANSWER_LIMIT} of {sectionBIdx.length} — only your first {NEET_SECTION_B_ANSWER_LIMIT} answered ({sectionBAnswered}/{NEET_SECTION_B_ANSWER_LIMIT} so far) will be scored.</span>}
              </div>
            )}
            <p className={`text-base sm:text-lg font-medium mb-4 leading-relaxed whitespace-pre-line ${(st.lang === 'bn' && q.textBn) ? 'bn' : ''}`}>
              {(st.lang === 'bn' && q.textBn) ? q.textBn : (q.textEn || 'This question could not be loaded.')}
            </p>
            {q.image && <img src={q.image} alt="Question diagram" className="max-h-64 rounded-lg mb-5 mx-auto" />}
        ...
        <div className={`w-full lg:w-72 shrink-0 card2 fixed lg:relative inset-y-0 right-0 z-[79] lg:z-auto transition-transform duration-300 ease-out flex flex-col ${paletteOpen ? 'translate-x-0' : 'translate-x-full'} lg:translate-x-0`} style={{ borderLeft: '1px solid var(--border)', maxWidth: '85vw' }}>
          <div className="flex-1 overflow-y-auto p-4 min-h-0">
            ...
            {hasSectionB ? (
              <>
                <p className="text-[11px] font-bold gold-text uppercase mb-2">Section A — Mandatory</p>
                ...
                <p className="text-[11px] font-bold text-purple-400 uppercase mb-2">Section B — Any {NEET_SECTION_B_ANSWER_LIMIT} of {sectionBIdx.length}</p>
                <div className="grid grid-cols-5 gap-2 mb-4">
                  {sectionBIdx.map((i) => (
                    <button key={i} onClick={() => { goToQuestion(i); setPaletteOpen(false); }} className={`palette-btn ${PALETTE_COLORS[st.status[i]]} ${st.current === i ? 'ring-2 ring-white' : ''}`}>
                      {i + 1}{st.status[i] === 'answered-marked' && <span className="absolute -top-1 -right-1 w-2 h-2 bg-emerald-400 rounded-full" />}
