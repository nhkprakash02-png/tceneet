...
export default function PyqManager() {
  ...
  const addNewPyqSet = () => {
    const title = prompt('PYQ Set title (e.g. "NEET UG 2025 - Physics Section"):'); if (!title) return;
    const examCategory = prompt('Exam category:', DB.examCategories[0] || 'NEET UG') || DB.examCategories[0];
    const year = prompt('Year (e.g. 2025):', '2025') || '2025';
    const durationInput = prompt('Test duration (in minutes):', '45');
    const durationMin = durationInput !== null && parseFloat(durationInput) > 0 ? parseFloat(durationInput) : 45;
    // Defaults to the official NEET marking scheme: +4 for correct, -1 for incorrect.
    const nt = { id: uid('pyq'), title, examCategory, year, durationMin, marksCorrect: 4, marksWrong: 1, questions: [] };
  ...
  const editPyqMeta = () => {
    ...
    const mc = prompt('Marks for correct answer (official NEET scheme: 4):', test.marksCorrect); if (mc === null) return;
    const mw = prompt('Negative marking (official NEET scheme: 1):', test.marksWrong); if (mw === null) return;
    updateSets((sets) => sets.map((t) => (t.id === activeTestId ? { ...t, title, examCategory, year, durationMin: parseFloat(dur) || t.durationMin, marksCorrect: parseFloat(mc) || t.marksCorrect, marksWrong: parseFloat(mw) >= 0 ? parseFloat(mw) : t.marksWrong } : t)));
  };
  const deleteTest = () => {
    if (!test) return;
