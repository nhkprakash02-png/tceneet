...
import { uid, SUBCATEGORIES, NEET_SUBJECTS } from '../../lib/utils';
...
export default function MockManager() {
  ...
  const addNewMockTest = () => {
    ...
    const examCategory = prompt('Exam category (e.g. "NEET UG"):', 'NEET UG') || 'NEET UG';
    const durationInput = prompt('Test duration (in minutes) — official NEET per-subject papers run 45 min, full papers run 180 min:', '45');
    ...
    const nt = { id: uid('mt'), subject, title, isDemo: false, adminUnlocked: false, examCategory, durationMin, subCategory, marksCorrect: 4, marksWrong: 1, questions: [], order: nextOrder };
  ...
  const editTestMeta = () => {
    ...
    const examCategory = prompt('Exam category (e.g. "NEET UG"):', test.examCategory || 'NEET UG'); if (examCategory === null) return;
    ...
    const mw = prompt('Negative marking for wrong answer (official NEET scheme: 1):', test.marksWrong); if (mw === null) return;
